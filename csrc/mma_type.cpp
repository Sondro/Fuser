...
size_t hash(const GemmTile& tile) {
  return (std::hash<size_t>{}(static_cast<size_t>(tile.m)) << 0) //
      ^ (std::hash<size_t>{}(static_cast<size_t>(tile.n)) << 1) //
      ^ (std::hash<size_t>{}(static_cast<size_t>(tile.k)) << 2);
}

size_t hash(const MatMulTileOptions& opts) {
  return (hash(opts.instruction_tile) << 0) //
      ^ (hash(opts.warp_tile) << 1) //
